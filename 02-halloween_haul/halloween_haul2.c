...
node *read_tree(char *);
node *read_tree_helper(char *, int *);
int tree_candy(node *);
int tree_streets(node *);
int max(int, int);
int tree_height(node *);
void tree_solve(node *);
...
node *
read_tree_helper(char *line, int *pos)
{
    node *tree;
